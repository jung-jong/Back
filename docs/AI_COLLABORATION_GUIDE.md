...
AI를 단순 코드 생성 도구로만 사용하지 않고, 요구사항 정리, 아키텍처 설계, API 계약 검토, 오류 원인 분석, 배포 점검까지 함께 수행하기 위한 기준으로 작성했습니다.
...
초기 서비스 단계에서는 문서 규모가 크지 않기 때문에 TiDB의 `document_chunks.embedding_json`에 embedding을 저장하고 애플리케이션 레벨에서 유사도를 계산합니다.
...
- 빠르게 시연 가능한 배포 방식을 우선하되, 보안상 민감정보는 코드에 남기지 않았습니다.
